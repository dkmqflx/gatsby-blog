...
Composition은 기존 클래스를 상속해서 확장하는 대신에 새로운 클래스를 만들고 필요한 곳에서 클래스의 인스턴스를 전달받아서 사용하는 방식을 말합니다.
...
그리고 TypeScript에는 하나 이상의 부모 클래스를 상속할 수 없기 때문에 과일 주스를 만들기 위한 `FruitJuiceMachine` 클래스와 야채 주스를 만들기 위한 `VegetableJuiceMachine` 클래스를 모두 상속하는 새로운 클래스를 만드는 것도 불가능합니다. 이러한 문제를 Composition을 통해 해결할 수 있습니다.
...
위 코드를 보면 과일 주스를 만들기 위한 `AddFruiteMaker` 인터페이스와 야채 주스를 만들기 위한 `AddVegetableMaker`라는 인터페이스를 정의해준 다음, 각각의 인터페이스를 구현하는 클래스들을 정의해주었습니다
